...
class TelegramBot:
    ...
    async def trigger_send_message(self, heartbeats: int):
        ...
        if heartbeats < 60:
            texto = f"Peligro: Ritmo cardiaco bajo {heartbeats} BPM. Medido a las {measure_time}"
        elif heartbeats > 100:
            texto = f"Peligro: Ritmo cardiaco alto {heartbeats} BPM. Medido a las {measure_time}"
        else:
            texto = f"Ritmo cardiaco normal {heartbeats} BPM. Medido a las {measure_time}"
