...
class Pulsesensor:
    ...
    def getBPMLoop(self):
        ...
        while not self.thread.stopped:
            ...
            # Filtrar ruido con una ventana deslizante (moving average filter)
    ...
    def filter_signal(self, signal):
        # Aplicar un filtro de media móvil simple
        window_size = 5
        if not hasattr(self, 'signal_window'):
            self.signal_window = [signal] * window_size
        else:
            self.signal_window.append(signal)
            if len(self.signal_window) > window_size:
